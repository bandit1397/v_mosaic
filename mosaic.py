...
# tracker 구조:
# {"roi": (x,y,w,h), "start": frame_idx, "end": frame_idx or None, "tracker": KCF tracker 객체}
trackers = []
...
def redraw_current():
    ...
    for t in trackers:
        if t["start"] <= current_idx and (t["end"] is None or current_idx <= t["end"]):
            processed_frames[current_idx] = apply_mosaic(processed_frames[current_idx], t["roi"])
